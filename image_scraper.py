...
def process_attractions():
    ...
    for json_file in json_files:
        try:
            ...
            # logger.info(
            #     f"{attraction_name}已有{existing_images}張圖片，將下載額外{images_to_download}張")
            ...
            for i, image_url in enumerate(image_urls[:images_to_download], 1):
                ...
                #     logger.info(
                #         f"{attraction_name}的圖片{existing_images + i}已存在，跳過")
                #     continue
                ...
                if success:
                    logger.info(
                        f"已儲存{attraction_name}的圖片 {i}/{images_to_download}")
                else:
                    logger.warning(
                        f"無法儲存{attraction_name}的圖片 {i}/{images_to_download}")
